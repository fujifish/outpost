  ...
  if (schema) {
    ...
    Object.keys(schema).forEach(function(key) {
      ...
      if (context[key] === undefined) {
        ...
        // if it's required but has no default, throw an error to stop processing
        if (context[key] === undefined && schema[key].required) {
          errors = errors.concat(' required parameter \'', key, '\' of type ', schema[key].type,  ', does not have a value.');
        }
      }
    });
