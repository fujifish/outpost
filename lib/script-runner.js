 ...
 * @param moduleName the submodule name
 * @param config the configuration to pass to the script
 * @param cb
 * @private
 */
ScriptRunner.prototype._runSubModuleScript = function(moduleName, config, cb) {
  var _this = this;

  this.manifest.moduleByName(moduleName, function(err, module) {
    if (err) {
      cb(err);
      return;
    }
    ...
    if (!module) {
      cb('module ' + moduleName +  ' not found');
      return;
    }
    ...
    if (!script) {
      cb('module ' + moduleName +  ' has no ' + _this.type + ' script');
      return;
    }
