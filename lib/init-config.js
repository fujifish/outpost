  ...
  function _prompt(defaults) {
    ...
    var schema = {
      properties: {
        ...
        syncFrequency: {
          description: 'Homebase sync frequency',
          message: 'Please specify how long to wait (in seconds, minimum is 30) between syncs with Homebase. Specify 0 to disable syncing',
          type: 'string',
    ...
    prompt.get(schema, function (err, result) {
      ...
      Object.keys(result).forEach(function(p) {
        if (typeof result[p] === 'string' && result[p].length === 0) {
          delete result[p];
        }
      });
