...
CommandUpdate.prototype._process = function(cb) {
  ...
  this.cache.download(newVersion, this.command.force, function(err, module) {
    ...
    _this.cache.unpack(newVersion, targetDir, function(err) {
      ...
      // delete the id so as not to update fortitude
      let commandId = _this.command._id;
      delete _this.command._id;
      ...
      setTimeout(function() {
        ...
        var updater = child.spawn(
          process.execPath,
          ['lib/updater.js', root, currentVersion, _this.command.version, commandId],
          { cwd: realCurrentDir,
            env: null,
