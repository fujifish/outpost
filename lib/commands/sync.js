    ...
    fortitude.sync(state, function(err, response) {
      ...
      _this.outpost.process(response.commands, function(err) {
        if (err) {
          ...
          // read the current state again to report it to fortitude immediately
          _this.state.current(function(err, state) {
            if (err) {
              cb('error', err);
              return;
            }
            ...
            fortitude.sync(state, function(err, response) {
              if (err) {
                _this.logger.debug('received error reporting re-syncing after state apply: ' + err);
                return;
              }
              cb('success');
            });
