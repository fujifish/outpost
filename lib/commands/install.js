...
CommandInstall.prototype._downloadModule = function(fullname, unpackDir, cb) {
  ...
  this.cache.moduleByFullName(fullname, function(err, module) {
    ...
    if (module) {
      ...
      var targetCacheDir = path.resolve(_this.command.outpost.config.cacheDir, parts.name + '/' + parts.version);
      if (downloader) {
        downloader.get(url, function(res) {
          ...
          cb('error', 'error downloading module ' + fullname + ': ' + e.message);
        });
      } else {
        fse.copy(_url.parse(url).path, targetCacheDir, function(err) {
          if (err) {
            cb('error', 'error copying module ' + fullname + ': ' + err.message);
            return;
          }
          _downloaded();
        });
...
CommandInstall.prototype._unpackModule = function(module, targetDir, cb) {
  ...
  fs.readdir(targetDir, function(err, files) {
    if (!err && files) {
      ...
      var cached = path.resolve(_this.command.outpost.config.cacheDir, module.name + '/' + module.version);
      fse.copy(cached, targetDir, function(err) {
        if (err) {
