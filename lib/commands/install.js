...
CommandInstall.prototype._downloadModule = function(fullname, unpackDir, cb) {
  var _this = this;
  this.cache.download(fullname, this.command.force, function(err, module) {
    if (err) {
      cb('error', err);
      return;
    }
...
CommandInstall.prototype._processModule = function(module, unpackDir, cb) {
  ...
  if (moduleData.outpost && !semver.satisfies(metadata.version, moduleData.outpost)) {
    cb('error', 'current outpost version (' + metadata.version + ') does not meet version requirement "' + moduleData.outpost + '"');
    return;
  }
...
CommandInstall.prototype._unpackModule = function(module, targetDir, cb) {
  ...
  fs.readdir(targetDir, function(err, files) {
    if (!err && files && files.indexOf('module.json') !== -1 && files.indexOf('.outpost') !== -1 && !_this.command.force) {
      // the directory structure seems good, no need to unpack the module
      _this.logger.debug('module ' + module.fullname + ' already unpacked');
      cb(null, false);
    } else {
      // continue with unpacking
      _this.cache.unpack(module.fullname, targetDir, function(err) {
        ...
        fs.mkdir(outpostDir, 0744, function(err) {
