...
CommandInstall.prototype._install = function(module, targetDir, cb) {
  ...
  if (installScript) {
    ...
    this.logger.info('running install script for module ' + module.fullname);
    var runner = new ScriptRunner('install', module, scriptName, installConfig, targetDir, this.command.outpost);
    runner.run(cb);
  } else {
    this.logger.debug('skipping install: module ' + module.fullname + ' has no install script');
