...
CommandStart.prototype._start = function(cb) {
  var _this = this;
  this.manifest.installedModule(this.command.module, this.command.outpost.config.root, function(err, module) {
    if (err) {
      cb('error', err);
      return;
    }
...
CommandStart.prototype._process = function(module, cb) {
  ...
  if (script) {
    ...
    this.logger.debug('skipping start: module ' + module.fullname + ' has no start script');
