...
Monitor.prototype.start = function(purge, cb) {
  ...
  this._load(function(err) {
    ...
    // before starting to monitor all processes, check which ones are already running
    // and erase the pid of those that are not running.
    // this is to prevent the situation of starting the processes one by one, and getting to check
    // a old process pid that has just been assigned to a new process that we created
    // thus ending up with a false positive
    procNames.forEach(function(name) {
      if (!_this._running(_this.db[name])) {
        delete _this.db[name].pid;
      }
    });

    // now start monitoring for all the monitored processes
    procNames.forEach(function(name) {
...
/**
 * Unmonitor all processes
 */
Monitor.prototype.unmonitorAll = function(cb, names) {
  if (names === undefined) {
    this.logger.debug('unmonitoring all processes');
    names = Object.keys(this.db);
    return this.unmonitorAll(cb, names);
  }

  if (names.length === 0) {
    this.logger.debug('done unmonitoring all processes');
    return cb();
  }

  let name = names.shift();
  this.unmonitor({name: name}, (err) => {
    this.unmonitorAll(cb, names);
  })
};
...
Monitor.prototype._save = function(proc, cb) {
  ...
  try {
    ...
    fs.writeFileSync(proc.pidFile, ''+proc.pid);
  } catch(err) {
...
Monitor.prototype._startMonitor = function(proc, timeout, cb) {
  ...
  this._startProcess(proc, timeout, function(err) {
    ...
    // register a timer to continuously check the process status
...
Monitor.prototype._registerMonitorTimer = function(proc, timeout, cb) {
  ...
  this.timers[proc.name] = new Timer(2000, function() {
    ...
    _this._startProcess(proc, timeout, function(err) {
      if (err) {
        _this.logger.error('error restarting process ' + proc.name + ': ' + err);
...
Monitor.prototype._startProcess = function(proc, timeout, cb) {
  ...
  try {
    ...
    child.on('error', function(err) {
      _this.logger.error('error occurred in monitored process ' + proc.name + ': ' + err.message);
    });
    if (!child.pid) {
      let message = 'pid of spawned process is ' + child.pid;
      _this.logger.error('error spawning process ' + proc.name + ': ' + message);
      cb && cb(message);
      return;
    }
  } catch(err) {
    _this.logger.error('error spawning process ' + proc.name + ': ' + err.message);
    cb && cb(err.message);
    return;
  } finally {
    fs.close(outFile);
  ...
  if (!proc.customPidFile) {
    ...
    function _check() {
      if (timeout > 0 && --timeout === 0) {
        cb && cb('timeout waiting for process ' + proc.name + ' to start');
        return;
      }
      try {
...
Monitor.prototype._checkMaxUpTime = function(check, proc) {
  ...
  if (check.time > 0 && upTime > check.time) {
    this.logger.warning('maxUpTime check failed: reached maximum up time of ' + check.time + ' minutes for ' + proc.name);
    return false;
  }
  return true;
};
...
Monitor.prototype._checkFileLastModified = function(check, proc) {
  ...
  // take into account if the file was last touched before the process even started
  var modifiedElapsed = (current - Math.max(stat.mtime.getTime(), proc.startedAt)) / 1000;
  if (modifiedElapsed > check.time) {
    this.logger.warning('fileLastModified check failed: file ' + file + ' was not modified for the last ' + modifiedElapsed + ' seconds');
    return false;
  }
  return true;
};
