...
Monitor.prototype._normalizeProcess = function(info, moduleFullpath) {
  ...
  proc.maxUpTime = info.maxUpTime || 0;
...
Monitor.prototype._startMonitor = function(proc, timeout, cb) {
  ...
  this._startProcess(proc, timeout, function(err) {
    ...
    _this.logrotator.register(proc.logFile, {schedule: '5m', size: '10m'});
...
Monitor.prototype._startProcess = function(proc, timeout, cb) {
  ...
  var upTime = (current - proc.startedAt) / (1000*60); // up time in minutes

  // restart the process if it reached the maximum up time
  if (running && proc.maxUpTime > 0 && upTime > proc.maxUpTime) {
    this.logger.info('reached maximum up time of ' + proc.maxUpTime + ' minutes for ' + proc.name + '. restarting.');
    try {
      process.kill(proc.pid, proc.stopSignal);
    } catch(err) {
      this.logger.error('process ' + proc.name + ' (' + proc.pid + ') could not be killed for restart: ' + err.message);
    }
    cb && cb();
    return;
  }
