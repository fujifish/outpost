...
Monitor.prototype.unmonitor = function(info, cb) {
  ...
  if (!proc) {
    this.logger.info('process '  + info.name + ' is already not monitored');
    cb && cb();
    return;
  }
...
Monitor.prototype._startProcess = function(proc, timeout, cb) {
  if (this._checkProcess(proc) || proc.starting) {
    cb && cb();
    return;
  }
  ...
  // if we have a custom pid file, delete it's content before trying to start it again
  ...
  function _processStarted(pid) {
    _this.logger.info('process ' + proc.name + ' started with pid ' + pid);
    proc.starting = false;
    proc.pid = pid;
    child.unref();
...
Monitor.prototype._stopProcess = function(proc, timeout, cb) {
  if (!proc.pid) {
    this.logger.info('process ' + proc.name + ' is already stopped');
    cb && cb();
    return;
  }

  this.logger.info('stopping process ' + proc.name + ' with pid ' + proc.pid);
  try {
  ...
  function _check() {
    ...
    if (_this._checkProcess(proc)) {
      setTimeout(function() {
        _check();
      }, 1000);
    } else {
      // yay, it's not running any more
      _this.logger.info('process ' + proc.name + ' stopped');
      proc.pid = null;
      cb && cb();
    }
  }
