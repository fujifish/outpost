 ...
 */
State.prototype.info = function(cb) {
  var self = this;
  utilities.diskspace(this.config.root, function(rootDS) {
    utilities.diskspace(__dirname, function(installedDS) {
      cb({
        ...
        diskspace: {
          root: rootDS,
          installed: installedDS
        },
        runas: process.getuid(),
        runasName: self.config.runasName,
        ip: self.config.ip,
        tags: self.tags
      });
    });
...
State.prototype.current = function(cb) {
  ...
  this.installed(function(err, modules) {
    ...
    _this.info(function(info) {
      var agentState = {
        info: info,
        modules: Object.keys(modules).map(function(name) {return modules[name];})
      };
      cb(null, agentState);
    });
  });
