        ...
        fs.readdir(path.resolve(moduleDir, '.outpost'), function(err) {
          ...
          if (!err) {
            ...
            _this.load(moduleDir, null, function(err, state) {
              ...
              _this.cache.moduleByFullName(module.fullname, function(err, metadata) {
                if (err) {
                  cb(err);
                  return;
                }
...
State.prototype.calculate = function(states, cb) {
  ...
  this.installed(function(err, installed) {
    ...
    states.forEach(function(state) {
      var fullname = state.fullname || state.name + '@' + state.version;
      ...
      if (configure) {
        ...
        if (configure.time && installedConfigure.time) {
          ...
          doConfigure = configHash !== installedConfigHash;
        }
      ...
      if (start !== undefined || configured) {
        var change;
        var changeTo;
        var startVal;

        var installedStart = installedState.start || {};
        var startedState = installedStart.data || {};
        var started = startedState.started;

        // compare times if given
        if (start !== undefined) {
          startVal = start.data ? start.data.started : start;
          if (start.time && installedStart.time) {
            change = Date.parse(start.time) > Date.parse(installedStart.time);
          } else {
            change = startVal !== started;
          }

          if (change) {
            changeTo = startVal ? 'start' : 'stop';
          }
        }
      }

      // if we're applying a new configuration and the process is started and should continue
      // to be started then we need to force a restart
      if (configured && started && changeTo === 'start') {
        ...
        commands.start.push({type: changeTo, module: fullname});
      }
    });
