...
Outpost.prototype._process = function(command, cb) {
  ...
  function callback(result, details) {
    if (command._id) {
      var captured = _this.logger.release();
      _this.homebase.updateCommand(command._id, {status: result, details: details, log: captured}, function(err) {
        if (err) {
  ...
  // capture the log of the command execution so we can report result to homebase
  if (command._id) {
    this.logger.capture();
  }
  ...
  if (!this.commands[command.type]) {
    ...
    callback('error', 'unknown command type ' + command.type);
    return;
  }
  ...
  commandObj.on('complete', function(result) {
    ...
    callback(result.result, result.details);
  });
