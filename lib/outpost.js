...
function Outpost(config) {
  ...
  this.config.runasName = utilities.uidToName(process.env['USER'] || process.getuid());
...
Outpost.prototype.start = function(cb) {
  ...
  function _startDone(err) {
    ...
    try {
      ...
      _this.logger.error('error writing ' + shaFile + ': ' + e.message);
    }

    // startup auto syncing with fortitude
    _this.fortitude.init(function() {
      if (err) {
        _this.logger.error('outpost agent start failed: ' + err);
      } else {
