...
function Orbital(config) {
  this.config = config || {};
  this.logger = new Logger('orbital', this.config.log);
}

Orbital.prototype.start = function(cb) {
  this.logger.info('starting orbital agent with config: ' + JSON.stringify(this.config));

  if (this.config.home) {
    this.home = url.parse(this.config.home);
    this.ssl = this.home.protocol === 'https:';
...
Orbital.prototype._loadCommands = function(cb) {
  this.commands = {};
  var _this = this;
...
Orbital.prototype._setupCallbacks = function(cb) {
  var _this = this;
...
Orbital.prototype._handleWorkerMessage = function(msg, cb) {
  if (msg.log) {
...
Orbital.prototype.stop = function() {
  cluster.emit('shutdown');
};

Orbital.prototype._fetchCommands = function() {
  var reqOptions = {
    hostname: this.home.hostname,
    path: (this.home.path || '/commands') + '?key=' + this.config.key,
    method: 'GET',
    proxy: this.proxy
  };
  var _this = this;
  var req = this.transport.request(reqOptions, function(res) {
    ...
    res.on('end', function(data) {
      ...
      _this.processCommands(JSON.parse(body));
    });
  });
...
Orbital.prototype.processCommands = function(commands, cb) {
  ...
  this._process(command, function(result) {
    _this.logger.info('command execution finished with ' + result.status);
    if (result.status !== 'success') {
      _this.logger.error('failure message: ' + result.message);
      _this.logger.error('aborting all later commands.');
    } else {
      _this.processCommands(commands);
    }
  });
};

Orbital.prototype._process = function(command, cb) {
  this.logger.info('processing command: ' + JSON.stringify(command));
  if (!this.commands[command.name]) {
    this.logger.error('error processing command ' + command.name + ': command not recognized');
    cb();
    return;
  }
...
exports = module.exports = Orbital;