...
function nodeExec() {
  var nodeFile = path.resolve(currentDir, 'node');
  try {
    log('debug', 'looking for node executable in ' + currentDir);
...
controlAgent('stop', function(err) {
  ...
  try {
    ...
    try {
      ...
      log('error', '*** update failed miserably. OUTPOST AGENT NOT RUNNING *** (failed reverting ' + oldDir + ' back to ' + currentDir + '): ' + err.message);
      return;
    }
