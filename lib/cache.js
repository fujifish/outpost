...
Cache.prototype.download = function(fullname, force, cb) {
  ...
  this.moduleByFullName(fullname, function(err, module) {
    ...
    var urlPath = parsed.path + parts.name + '/' + name + (suffix ? ('/' + name + '-' + process.platform + suffix) : '');
    var urlPathGeneric = parsed.path + parts.name + '/' + name + (suffix ? ('/' + name + suffix) : '');
    ...
    fse.ensureDir(targetCacheDir, function(err) {
      ...
      if (downloader) {
        _this.logger.debug('trying to download from ' + registry + urlPath);
        var options = {
        ...
        downloader.get(options, function(res) {
          if (res.statusCode >= 400) {
            ...
            _this.logger.debug('not found. trying to download from ' + registry + urlPathGeneric);
        ...
        fs.open(url, 'r', function(err, fd) {
          if (err) {
            _this.logger.debug(url + ' not found. trying to download from ' + urlGeneric);
            fs.open(urlGeneric, 'r', function(err, fd) {
              if (err) {
                _this.logger.debug(urlGeneric + ' not found');
