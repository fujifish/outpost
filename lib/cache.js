...
Cache.prototype.download = function(fullname, force, cb) {
  ...
  this.moduleByFullName(fullname, function(err, module) {
    ...
    fse.ensureDir(targetCacheDir, function(err) {
      ...
      var counter = new Writable({
        write: function(chunk, encoding, callback) {
          counter.received += chunk.length;
          var total = `${counter.received}/${counter.expected ? counter.expected : '?'} bytes`;
          if (counter.expected) {
            let percent = Math.floor(counter.received / counter.expected * 100);
            if (percent !== counter.percent) {
              counter.percent = percent;
              _this.logger.debug(`downloaded ${counter.percent}% (${total})`);
            }
          } else {
            _this.logger.debug(`downloaded ${total}`);
          }
          callback();
        }
      });
      ...
      tgzStream.on('close', function() {
        if (counter.expected && counter.expected !== counter.received) {
          _this._moduleDownloadFailed(tgz, 'error downloading module ' + fullname + ': expected ' + counter.expected + ' bytes but received ' + counter.received + ' bytes', cb);
          cb = null;
          return;
        }
      ...
      tgzStream.on('error', function(e) {
        _this._moduleDownloadFailed(tgz, 'error downloading module ' + fullname + ': ' + e.message, cb);
        cb = null;
      });
          ...
          if (res.statusCode >= 400) {
            ...
            downloader.get(downloadOptions, function(res) {
              if (res.statusCode >= 400) {
                tgzStream.end();
                res.destroy();
                _this._moduleDownloadFailed(tgz, 'error downloading module ' + fullname + ': module not found', cb);
                cb = null;
                return;
              }

              counter.expected = parseInt(res.headers['content-length']);
              _this.logger.debug('found module ' + fullname + '. starting download (' + counter.expected  +' bytes)');
              res.pipe(counter);
              res.pipe(tgzStream);
 ...
 * @private
 */
Cache.prototype._moduleDownloadFailed = function(file, error, cb) {
  var _this = this;
  fs.unlink(file, function(err) {
    if (err && err.code !== 'ENOENT') {
      _this.logger.error('failed to remove ' + file + ': ' + err.message);
    }
    cb && cb(error);
  });
};
