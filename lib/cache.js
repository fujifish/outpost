...
Cache.prototype.download = function(fullname, force, cb) {
  ...
  this.moduleByFullName(fullname, function(err, module) {
    ...
    var url = registry + '/' + parts.name + '/' + name + (suffix ? ('/' + name + '-' + process.platform + suffix) : '');
    var urlGeneric = registry + '/' + parts.name + '/' + name + (suffix ? ('/' + name + suffix) : '');
    ...
    fse.ensureDir(targetCacheDir, function(err) {
      ...
      if (downloader) {
        _this.logger.debug('trying to download from ' + url);
        downloader.get(url, function(res) {
          if (res.statusCode >= 400) {
            // the platform specific module not found, try to download the generic one
            _this.logger.debug('not found. trying to download from ' + urlGeneric);
            downloader.get(urlGeneric, function(res) {
              if (res.statusCode >= 400) {
