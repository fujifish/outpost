...
Logger.prototype.redirectIO = function() {
  process.stderr.write = (line, next) => {
    this.log('STDERR', [line.trim()], next);
  };
  process.stdout.write = (line, next) => {
    this.log('STDOUT', [line.trim()], next);
  };
};
...
Logger.prototype.write = function(message, next) {
  var m = message;
  if (this.isFile) {
    m = chalk.stripColor(message);
  }
  this.stream.write(m, next);
